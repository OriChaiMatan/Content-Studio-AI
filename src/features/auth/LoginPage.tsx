  ...
  return (
    <AuthLayout>
      <header className="space-y-1">
        <h2 className="font-serif text-[32px] leading-10 text-on-surface">{t('auth.signIn')}</h2>
        <p className="text-[14px] text-on-surface-variant">{t('auth.signInSubtitle')}</p>
      </header>
      ...
      <form onSubmit={handleSubmit} className="space-y-lg">
        ...
        />
        <AuthField
          id="password" label={t('auth.password')} type={show ? 'text' : 'password'} autoComplete="current-password" required
      ...
      <p className="text-[14px] text-on-surface-variant">
        {t('auth.noAccount')}{' '}
        <Link to="/register" className="font-bold text-primary hover:underline underline-offset-4">{t('auth.createOne')}</Link>
      </p>
    </AuthLayout>
  );
}
