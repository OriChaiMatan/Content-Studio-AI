...
export function SubpageChrome({ eyebrow, title, children }: { eyebrow: string; title: string; children: ReactNode }) {
  return (
    <div style={{ background: '#0D1121', color: '#E9EBFF', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <nav style={{ height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 40px', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
      ...
      <div style={{ flex: 1, maxWidth: 680, margin: '0 auto', padding: '100px 40px 120px', width: '100%' }}>
        ...
        <h1 style={{ fontSize: 44, fontWeight: 700, lineHeight: 1.15, margin: '0 0 24px' }}>{title}</h1>
        {children}
      </div>
      <footer style={{ borderTop: '1px solid rgba(255,255,255,0.07)', padding: '32px 40px', textAlign: 'center', fontSize: 12, color: '#606880' }}>© 2026 LumAI</footer>
    </div>
  );
}
