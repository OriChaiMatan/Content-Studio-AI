...
export function engineSystem(lang: 'en' | 'he'): string {
  ...
  return [
    ...
    '- PUNCTUATION — NEVER use em dashes (the "—" character) anywhere in the output. Rewrite with a comma, a period, or parentheses instead (e.g. "the idea, which is great, works" or split into two sentences). This applies to readyToPublish and every breakdown field, in both English and Hebrew. Regular hyphens in compound words and en dashes in number ranges are fine; only the em dash is banned.',
